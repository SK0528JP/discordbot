...
class Study(commands.Cog):
    ...
    async def study_start(self, interaction: discord.Interaction):
        ...
        if user_data.get("is_studying"):
            await interaction.response.send_message("⚠️ すでに学習中です！", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"📚 {interaction.user.display_name}さん、学習を開始しました！頑張ってください。")
    ...
    async def study_end(self, interaction: discord.Interaction):
        ...
        if not user_data.get("is_studying"):
            await interaction.response.send_message("⚠️ まだ学習を開始していません！", ephemeral=True)
            return
        ...
        if minutes < 1:
            ...
            self.bot.ledger.save()
            await interaction.response.send_message("⏱️ 1分未満の学習は記録されません。お疲れ様でした！")
            return
        ...
        # 学習1分につき1xpと0.5cr（端数切り捨て）を付与するなどの報酬設定も可能
        reward_cr = minutes // 2
        ...
        await interaction.response.send_message(
            f"✅ お疲れ様でした！\n"
            f"📖 今回の学習時間: **{time_str}**\n"
            f"💰 報酬: **{reward_cr} cr** / **{minutes} xp** を獲得しました！"
        )
    ...
    async def study_stats(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"📊 {interaction.user.display_name} の学習統計", color=discord.Color.green())
        
        th, tm = divmod(today_min, 60)
        all_h, all_m = divmod(total_min, 60)
        
        embed.add_field(name="今日の学習時間", value=f"{th}時間{tm}分" if th > 0 else f"{tm}分", inline=True)
        embed.add_field(name="累計学習時間", value=f"{all_h}時間{all_m}分", inline=True)
    ...
    async def study_ranking(self, interaction: discord.Interaction, span: str = "total"):
        ...
        if span not in ["daily", "weekly", "monthly", "total"]:
            await interaction.followup.send("❌ 引数は daily, weekly, monthly, total のいずれかを指定してください。")
            return

        ranking_data = []
        now = datetime.now(JST)

        for user_id_str, data in self.bot.ledger.data.items():
            try:
            ...
            if span == "total":
                ...
                for date_str, minutes in history.items():
                    try:
                        log_date = datetime.strptime(date_str, "%Y-%m-%d").replace(tzinfo=JST)
                        if span == "daily" and log_date.date() == now.date():
                            time_val += minutes
                        elif span == "weekly" and now - log_date < timedelta(days=7):
                            time_val += minutes
                        elif span == "monthly" and now - log_date < timedelta(days=30):
                            time_val += minutes
                    except ValueError:
                        continue
        ...
        if not ranking_data:
            await interaction.followup.send(f"⚠️ {span} のランキングデータはありません。")
            return

        ranking_data.sort(key=lambda x: x["time"], reverse=True)

        embed = discord.Embed(
            title=f"🏆 学習ランキング ({span})",
